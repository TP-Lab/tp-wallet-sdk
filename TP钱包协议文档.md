...
~~~
   ...
   callbackUrl string //回调url，通过此url 将txId和actionId通过post请求告诉dappServer
   callbackSchema    string     callbackSchema //可选，集成TP转账SDK，不需要关注这个字段，授权登陆后，回调通知DAPP，这里采用schema://xxx/xxx?param={
        result: 返回结果，0 成功，1 失败 2 取消
        message://结果描述，可选
        data:{txId:xxx,actionId:xxx}
    }
  compressedData string //压缩后的协议内容，如果使用了压缩算法，则该字段表示整个json字符串压缩后的内容，如果没有压缩，该字段可以为空将transactionId和actionId通过post请求告诉dappServer    
  compress  number //对协议内容压缩方式 0 表示不压缩 其他待定
~~~
拉起支付使用详情：https://github.com/TP-Lab/tp-wallet-android
